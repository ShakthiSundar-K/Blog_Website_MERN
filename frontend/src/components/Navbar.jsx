...
const Navbar = () => {
  ...
  const location = useLocation(); // Add location hook to track current path
  ...
  // Navigation links with dynamic active state based on current location
  const navLinks = [
    { name: "Home", path: "/" },
  ...
  const navbarHeight = isScrolled ? "64px" : "80px"; // Approximate heights based on py-2 vs py-4
      ...
      >
        ...
        >
          <div className='px-4 py-2 space-y-1 border-t'>
            {navLinks.map((link) => {
              // Check if this link is active based on current path for mobile menu
              const isActive =
                (link.path === "/" && location.pathname === "/") ||
